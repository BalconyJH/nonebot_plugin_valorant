...
async def fetch_json_data(data: str, proxy=plugin_config.valorant_proxies) -> Optional[Dict]:
...
async def get_playercards() -> Optional[Dict]:
    ...
    try:
        ...
        if resp:
            return {
                card['uuid']: parse_playercard_data(card)
...
async def get_player_titles() -> Optional[Dict]:
    ...
    try:
        ...
        if resp:
            return {
                title['uuid']: parse_title_data(title)
...
async def get_spray() -> Optional[Dict]:
    ...
    try:
        ...
        if resp:
            return {
                spray['uuid']: parse_spray_data(spray)
